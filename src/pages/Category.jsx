...
export default function Category() {
  ...
  const nombreFiltro = meta ? meta.nombre : slug;

  const sorted = useMemo(() => {
    let arr = products.filter((p) =>
      p.NombreCategoria?.toLowerCase() === nombreFiltro?.toLowerCase()
    );
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-5 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        <div>
          <div className="label-tiny mb-2">CATEGORÍA</div>
          <h1 className="font-display text-[clamp(2.4rem,5vw,4rem)] uppercase leading-none">
            {meta?.nombre?.toUpperCase() || slug?.toUpperCase()}
          </h1>
      ...
      {totalItems === 0 ? (
        <div className="border-2 border-black p-12 text-center">
          ...
          <div className="text-[var(--muted)] text-[0.9rem]">Aún no hay productos en esta categoría.</div>
        </div>
      ) : (
